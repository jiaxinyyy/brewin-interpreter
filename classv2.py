...
class TClassDef:
    def __init__(self, class_source, interpreter):
        ...
        self.parameterized_types = class_source[2] # a list of parameterized types 
...
class ClassDef:
    ...
    def __create_variable_def_from_field(self, field_def):
        ...
        class_and_types = field_def[1].split((InterpreterBase.TYPE_CONCAT_CHAR))
        if class_and_types[0] in self.interpreter.tclass_index:
            ...
            if field_def[1] not in self.interpreter.type_manager.map_typename_to_type:
        ...
        field_val = None
        if len(field_def) == 3:
            field_val = create_default_value(field_type)
        else:
            field_val = create_value(field_def[3])
        var_def = VariableDef(
            field_type, field_def[2], field_val
        )
