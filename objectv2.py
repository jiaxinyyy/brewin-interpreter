...
class ObjectDef:
    ...
    def __add_locals_to_env(self, env, var_defs, line_number):
        for var_def in var_defs:
            ...
            # if it is a parameterized template class (e.g. node@int)
            class_and_types = var_def[0].split((InterpreterBase.TYPE_CONCAT_CHAR))
            if class_and_types[0] in self.interpreter.tclass_index:
                tclass_def = self.interpreter.tclass_index[class_and_types[0]]
                # add this parameterized type to typemanager
                if var_def[0] not in self.interpreter.type_manager.map_typename_to_type:
                    # check if the number of concrete types are equal to the number of parameterized types
                    if len(class_and_types[1:]) != len(tclass_def.parameterized_types):
                        self.interpreter.error(
                            ErrorType.TYPE_ERROR,
                            "invalid type/type mismatch with field " + var_def[1],
                                var_def[1].line_num,
                        )
                    # check if the concrete type is valid
                    for t in class_and_types[1:]:
                        # print(t)
                        if t not in self.interpreter.type_manager.primitive_types and t not in self.interpreter.class_index:
                            self.interpreter.error(
                                ErrorType.TYPE_ERROR,
                                "invalid type/type mismatch with variable " + var_def[1],
                                var_def[1].line_num,
                            )
    ...
    def __execute_throw(self, env, return_type, code):
        # ex. code: ['throw', '"blah"']
        # check the parameter to the throw statement is of the string type
        if isinstance(code[1], list):
            param_value = self.__evaluate_expression(env, code[1], code[0].line_num)
        else:
            param_value = create_value(code[1])
        if param_value.type().type_name == InterpreterBase.STRING_DEF:
            # [return status of “throw” and return_value]
            return ObjectDef.STATUS_EXCEPTION, param_value
        self.interpreter.error(
            ErrorType.TYPE_ERROR, "wrong throw parameter", code[0].line_num
        )
     
    def __execute_try(self, env, return_type, code):
        status, return_value = self.__execute_statement(env, return_type, code[1])
        if status == ObjectDef.STATUS_EXCEPTION:
            # env.block_nest() # later add this to make it only local available
            env.create_new_symbol(InterpreterBase.EXCEPTION_VARIABLE_DEF)
            env.set(InterpreterBase.EXCEPTION_VARIABLE_DEF, return_value)
            return self.__execute_statement(env, return_type, code[2])
        # if no exception, proceed to next statement outside try statement
    ...
    def __execute_call(self, env, code):
        return_value = self.__execute_call_aux(
            env, code, code[0].line_num
        )
        if isinstance(return_value, tuple):
            return ObjectDef.STATUS_EXCEPTION, return_value[1]
    ...
    def __execute_set(self, env, code):
        ...
        # check if the set statement throws an exception
        if isinstance(val, tuple):
            return val
        result = self.__set_variable_aux(
            env, code[1], val, code[0].line_num
        )  # checks/reports type and name errors
        if result is not None: # exception
            return result
        return ObjectDef.STATUS_PROCEED, None

    # (return expression) where exprObjectDef.STATUSesion could be a value, or a (+ ...)
    ...
    def __execute_if(self, env, return_type, code):
        ...
        # check if the condition throws an exception
        if isinstance(condition, tuple) and condition[0] == ObjectDef.STATUS_EXCEPTION:
            return ObjectDef.STATUS_EXCEPTION, condition[1]
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement):
        if type(expr) is not list:
            ...
            if expr == InterpreterBase.EXCEPTION_VARIABLE_DEF and var_def is not None:
                return var_def
    ...
    def __execute_call_aux(self, env, code, line_num_of_statement):
        ...
        actual_args = []
        for expr in code[3:]:
            # could be nested calls "(call me bar (call me foo))"
            # check if the argument here, (call me foo), will throw an exception
            arg = self.__evaluate_expression(env, expr, line_num_of_statement)
            if isinstance(arg, tuple):
                return ObjectDef.STATUS_EXCEPTION, arg[1]
